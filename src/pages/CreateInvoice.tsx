...
const CreateInvoice = () => {
  ...
  const { seller, customers, addInvoice, addCustomer, nextInvoiceNumber } = useAppStore();
  ...
  const handleAddNewCustomer = async () => {
    ...
    const c = await addCustomer(userId, newCust);
    selectCustomer(c.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!customers.find((c) => c.name === clientName)) {
      await addCustomer(userId, { name: clientName, phone: clientPhone, email: clientEmail, address: clientAddress, gstin: clientGstin, placeOfSupply });
    }
